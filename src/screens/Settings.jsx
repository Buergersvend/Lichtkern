...
function PinLock({ mode, onSuccess, onSetup, onLogout, userId }) {
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:9999,background:T.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px"}}>
      <div style={{width:"72px",height:"72px",borderRadius:"50%",background:`linear-gradient(135deg,${T.gold},${T.goldL})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"30px",boxShadow:`0 6px 28px ${T.shadowDeep}`,border:`1.5px solid ${T.border}`,marginBottom:"20px"}}>
        🔒
      </div>
          ...
          {netzwerk.sichtbar && <>
            ...
            <div style={{marginBottom:"14px"}}>
              ...
              <select onChange={e=>{const v=e.target.value;if(v&&!netzwerk.methoden.includes(v))setNetzwerk(n=>({...n,methoden:[...n.methoden,v]}));e.target.value="";}} style={{width:"100%",background:T.bgCard,border:`1.5px solid ${T.borderMid}`,borderRadius:"8px",padding:"8px",color:T.textSoft,fontFamily:"Raleway",fontSize:"11px"}}>
                ...
                {["Quantenarbeit","Systemische Arbeit","Körperarbeit","Energiearbeit","Naturheilkunde","Psychotherapie & Beratung","Spirituelle Begleitung","Sonstiges"].filter(m=>!netzwerk.methoden.includes(m)).map(m=>(<option key={m} value={m}>{m}</option>))}
              </select>
