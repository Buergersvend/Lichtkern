...
function SC({wizard,clients,upd}){return(<div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
  ...
  <div><SL>Sitzungstyp</SL><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px"}}>{[["first","🌱 Erst"],["followup","🔄 Folge"],["closing","✨ Abschluss"]].map(([v,l])=><button key={v} onClick={()=>upd({type:v})} style={{padding:"11px 6px",borderRadius:"12px",border:`1.5px solid ${wizard.type===v?T.gold:T.border}`,background:wizard.type===v?T.gold:T.bgCard,cursor:"pointer",fontFamily:"Raleway",fontSize:"11px",fontWeight:700,color:wizard.type===v?"white":T.textMid}}>{l}</button>)}</div></div>
</div>);}
...
function ST({wizard,upd,t2}){
  ...
  const sugg=[...new Set([...(keys.some(k=>["emotional","mental"].includes(k))?["Emotionale Transformation","Mentale Muster"]:[]),...(keys.includes("dna")?["DNA & Ahnen","Zeitlinie & Karma"]:[]),...(keys.includes("energetisch")?["Meridian & Chakren"]:[]),...(keys.includes("spirituell")?["Schamanismus","Arbeit auf Distanz"]:[]),...(keys.some(k=>["struktur","stoffwechsel"].includes(k))?["Analyse & Bestandsaufnahme"]:[])])];
...
function SA({wizard,upd,aiText,aiLoading,onGen}){
  ...
  <SL color={T.goldD}>💰 Honorar</SL>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",marginBottom:"10px"}}>
      ...
      <div>
        <select value={wizard.payStatus||"open"} onChange={e=>upd({payStatus:e.target.value})} style={{width:"100%",background:T.bgCard,border:`1.5px solid ${T.borderMid}`,borderRadius:"8px",padding:"8px",color:T.textSoft,fontFamily:"Raleway"}}>
  <option value="open">🧾 Offen</option>
  <option value="partial">⚠️ Teilbezahlt</option>
  <option value="energieausgleich">🌀 Energieausgleich</option>
